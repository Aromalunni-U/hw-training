...
url = "https://www.franprix.fr/department/boissons"

response = requests.get(url=url, headers=HEADERS)
if response.status_code == 200:
    sel = Selector(response.text)
    
    pagination = sel.xpath('//div[@class="hidden"]/a/@href').getall()
    full_page = f"https://www.franprix.fr{pagination[-1]}"
    ...
    if res.status_code == 200:
        sec_sel = Selector(res.text)
        pdp_links = sec_sel.xpath('//a[@class="product-card__title"]/@href').getall()
        
        pdp_links = [f"https://www.franprix.fr{link}" for link in pdp_links]
